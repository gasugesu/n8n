	...
	describe('webhook method', () => {
		describe('signature verification', () => {
			...
			it('should verify valid signature correctly', async () => {
				...
				Date.now = jest.fn(() => parseInt(testTimestamp, 10) * 1000 + 1000); // 1 second later
			...
			it('should reject invalid signature', async () => {
				...
				// Mock current time to be close to test timestamp
				const originalDateNow = Date.now;
				Date.now = jest.fn(() => parseInt(testTimestamp, 10) * 1000 + 1000);
			...
			it('should handle credentials error gracefully (skip signature verification)', async () => {
				context.getCredentials.mockRejectedValue(new Error('Credentials error'));
				...
				// Should process the webhook without signature verification when credentials are not available
				expect(result).toEqual({
					workflowData: [
						[
							{
